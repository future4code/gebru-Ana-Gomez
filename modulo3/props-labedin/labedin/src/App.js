import React from 'react';
import './App.css';
import CardGrande from './components/CardGrande/CardGrande';
import CardPequeno from './components/CardPequeno/CardPequeno';
import ImagemButton from './components/ImagemButton/ImagemButton';


function App() {
  return (
    <div className="App">
      <div className="page-section-container">
        <h2>Dados pessoais</h2>
        <CardGrande 
          imagem="https://i.ibb.co/gVwRRwZ/foto-eu.jpg" 
          nome="Ana Flávia" 
          descricao="Olá! Eu sou a Ana, aluna da Labenu. Adoro desenvolvimento web e assuntos relacionados a tecnologia.
          Além disso, curso Bacharel em Ciências Sociais. Sou movida por desafios e aprender coisas novas."
        />
        
        <ImagemButton 
          imagem="https://image.flaticon.com/icons/png/512/117/117472.png" 
          texto="Ver mais"
        />
      </div>

      <div className='page-section-container'>
        <CardPequeno
        imagem="http://cdn.onlinewebfonts.com/svg/img_237869.png"
        texto="E-mail: csoanaufscar@outlook.com"
        />

        <CardPequeno
        imagem="https://i.pinimg.com/736x/a7/94/40/a79440a6b5663f44370231c55bffb637.jpg"
        texto="Endereço: Rua Olinda, 193, Ribeirão Preo"
        />
      </div>

      <div className="page-section-container">
        <h2>Experiências pessoais</h2>
        <CardGrande 
          imagem="data:image/png;base64," 
          nome="Labenu" 
          descricao="Aluna de desenvolvimento web full stack 2021." 
        />
        
        <CardGrande 
          imagem="https://logodownload.org/wp-content/uploads/2015/02/ufscar-logo-4.png" 
          nome="UFSCar" 
          descricao="Aluna de bacharel em Ciências Sociais desde 2017." 
        />
      </div>

      <div className="page-section-container">
        <h2>Minhas redes sociais</h2>
        <ImagemButton 
          imagem="https://d2v9ipibika81v.cloudfront.net/uploads/sites/261/2017/01/facebook-logo-3.png" 
          texto="Facebook" 
        />        

        <ImagemButton 
          imagem="https://logodownload.org/wp-content/uploads/2014/09/twitter-logo-1-1.png" 
          texto="Twitter" 
        />        
      </div>
    </div>
  );
}

export default App;
